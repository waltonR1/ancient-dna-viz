...
#     return mat
#
# def _read_geno_packed(path: Path, expected_nind: int, expected_nsnps: int, missing_code_packed: int = 3, dtype=np.int8) -> np.ndarray:
#     """
...
#     return out
#
# def read_geno(path: str | Path, missing_code: int | str = 9, dtype=np.int8, expected_nind: int | None = None, expected_nsnps: int | None = None) -> np.ndarray:
#     """
...
#
# def eigenstrat_to_dataframe(geno_mat: np.ndarray, snp: pd.DataFrame, ind: pd.DataFrame, missing_code: int = 9, missing_as: float | None = np.nan) -> Tuple[pd.DataFrame, pd.Series, List[str]]:
#     """
...
#
# def load_eigenstrat(prefix: str | Path, anno_path: str | Path | None = None, missing_code: int = 9, missing_as: float | None = np.nan, id_col_meta: str = "Genetic ID") -> Tuple[pd.Series, pd.DataFrame, List[str], pd.DataFrame | None]:
#     """
