...
该模块属于分析流程中的聚类分析模块（Clustering Layer）， 用于在 高维基因型矩阵 或 降维嵌入空间（UMAP / t-SNE / PCA） 中执行 层次聚类（Hierarchical Clustering），并支持：
- 自动搜索最优聚类数（Silhouette Score）
- 在高维 / 低维空间执行聚类
- 计算并评估聚类与真实标签之间的一致性（聚类纯度）

### 📋 函数总览
|                函数名                |           功能简介            |
|:---------------------------------:|:-------------------------:|
| `find_optimal_clusters_embedding` |      基于轮廓系数自动搜索最优聚类数      |
...
|     `metric`     |     `str`      | `"euclidean"`  |                    距离度量方式（适用于二进制或基因型矩阵）。                    |
...
6. 当某个 k 的聚类结果退化为单一簇时，其轮廓系数记为 -1。
...
* 分数越高，聚类效果越好；
* 适用于小至中型数据集的自动聚类优化；
* 可配合 [`plot_silhouette_trend()`](#75-plot_silhouette_trend) 一同使用，以可视化评估最优聚类数。
* 当 `linkage_method="ward"` 时，仅支持 `"euclidean"` 距离度量。
...
* 结果可结合 [`plot_cluster_on_embedding()`](#74-plot_cluster_on_embedding) 可视化聚类表现；
...
* **meta_aligned**：在原元数据表基础上新增 `"cluster"` 列，记录每个样本在降维空间中的聚类结果。
...
* 聚类结果新增列 `"cluster"`，与 `meta` 表索引顺序保持一致；
* 结果可配合 [`plot_cluster_on_embedding()`](#74-plot_cluster_on_embedding) 进行聚类分布可视化；
...
| `cluster_col` |     `str`      |  `"cluster"`   | 聚类结果所在列名（如由 `cluster_on_embedding` 生成）。  |
...
* 可配合 [`plot_cluster_on_embedding()`](#74-plot_cluster_on_embedding) 一起使用，进行可视化验证；
...
|    `method`    |     `str`      | `umap` | 降维方法：`'umap'`, `'tsne'`, `'mds'`, `'isomap'` |
...
- **X_filtered**: 过滤后的矩阵
- **keep_rows**: 样本级布尔掩码，用于同步过滤 meta（可配合 filter_meta_by_rows 使用）
...
该函数通常与 filter_by_missing 配合使用，用于在过滤高缺失率样本后，对元数据表（meta）执行一致的样本筛选，以保持基因型矩阵与元数据之间的行对齐关系。

**参数：**

|     参数名     |       类型       |  是否默认  |         说明          |
|:-----------:|:--------------:|:------:|:-------------------:|
|   `meta`    | `pd.DataFrame` |        |   样本元数据表（行 = 样本）。   |
| `keep_rows` |  `pd.Series`   |        | 样本级布尔掩码（True 表示保留）。 |
|   `label`   |     `str`      | `meta` |    日志输出中使用的名称标识     |

**返回：**

`(meta_filtered: pd.DataFrame)`
- `meta_filtered`：按 keep_rows 过滤后的元数据表，行顺序与过滤后的基因型矩阵一致。

**算法逻辑：**
1. 接收由 filter_by_missing 返回的样本级布尔掩码 keep_rows；
2. 使用该掩码对元数据表 meta 进行行过滤；
3. 返回过滤后的元数据表，用于后续分析流程。
...
**说明：**
- 该函数用于保证 基因型矩阵与元数据表在样本维度上的严格一致性；
- 推荐始终在 filter_by_missing 之后调用，以避免样本错位；
- 这是一个辅助函数，不对数据本身进行修改，仅执行行级筛选；
- 常用于后续的 降维、聚类与可视化分析流程。
...
|:-------------:|:--------------:|:--------:|:--------------------------------------------------------------------------------------------:|
|      `X`      | `pd.DataFrame` |          |                                            基因型矩阵                                             |
|   `method`    |     `str`      | `"mode"` | 填补方法（`mode`, `mean`, `knn`, `knn_hamming_abs`,`knn_hamming_adaptive`,`knn_hamming_balltree`） |
...
该函数对分类标签进行标准化与过滤，并根据保留下来的有效样本索引，同步筛选并对齐降维后的 embedding，确保二者在样本维度上的一致性。过滤完成后，embedding 与 labels 会同时重建为连续整数索引。

**参数：**

|       参数名        |         类型         |  是否默认   |                     说明                     |
|:----------------:|:------------------:|:-------:|:------------------------------------------:|
|      `emb`       |   `pd.DataFrame`   |         |             降维后的 embedding 矩阵              |
|     `labels`     |    `pd.Series`     |         |           分类标签序列，索引需与 emb 行索引对应。           |
|   `collapse_y`   |       `bool`       | `False` | 是否将 Y 单倍群标签映射为更高层级大类（仅对 Y haplogroup 场景生效） |
| `invalid_values` | `iterable \| None` | `None`  |   需视为无效并过滤掉的标签集合；若为 None，使用函数内部默认无效值集合。    |
|   `whitelist`    | `iterable \| None` | `None`  |        若提供，仅保留白名单中的标签类别（匹配时不区分大小写）。        |


**返回：**

`(emb_clean: pd.DataFrame, labels_clean: pd.Series)` 
- **emb_clean**：过滤并对齐后的 embedding 矩阵，已执行 reset_index(drop=True)。
- **labels_clean**：与 emb_clean 行顺序一致的标签序列，已执行 reset_index(drop=True)。

**算法逻辑：**
1. 接收当前样本子集对应的基因型矩阵 X；
2. 基于 X 的索引对子标签 labels 进行重新索引与对齐；
3. 移除无效、缺失或无法对齐的标签项；
4. 返回与样本顺序一致的标签序列，用于后续分析。
...
|             函数名              |              功能简介               |
|:----------------------------:|:-------------------------------:|
|       `plot_embedding`       |      绘制降维结果散点图（支持 2D / 3D）      |
| `plot_embedding_interactive` | 绘制交互式 2D / 3D embedding（Plotly） |
...
绘制降维结果散点图，支持 二维 / 三维（2D / 3D） embedding 的可视化，并支持自定义图例位置与颜色映射。
当 embedding 中包含 Dim3 列时，函数将自动绘制三维散点图。
...
| `draw_others`  |        `bool`         |        `False`         |        是否在图中绘制被归并为 "others" 的点。         |
...
绘制 交互式 embedding 可视化（Plotly）。

该函数基于 Plotly 绘制二维或三维 embedding 的交互式散点图。
每个类别对应一个独立的 Plotly trace，从而支持通过图例对不同类别进行显示 / 隐藏控制，适用于交互式探索与展示。

**参数：**

|      参数名       |          类型           |        是否默认         |                          说明                           |
|:--------------:|:---------------------:|:-------------------:|:-----------------------------------------------------:|
|      `df`      |    `pd.DataFrame`     |                     | 降维后的 embedding 数据，必须包含 `Dim1`、`Dim2`，三维情况下需包含 `Dim3`。 |
|    `labels`    |      `pd.Series`      |                     |              用于着色的分类标签，索引需与 `df` 行顺序一致。               |
|     `dim`      |     `int \| None`     |       `None`        | 指定 embedding 维度（2 或 3）；若为 `None`，自动根据是否存在 `Dim3` 推断。  |
|    `title`     |         `str`         |   `"Projection"`    |                         图标题。                          |
|  `legend_max`  |         `int`         |        `20`         |            图例中最多显示的类别数量，超出部分合并为 “others”。             |
| `legend_sort`  |        `bool`         |       `False`       |                是否按类别出现频次排序（传递给颜色映射逻辑）。                |
|     `cmap`     |         `str`         |      `"tab20"`      |           普通离散类别使用的 Matplotlib colormap 名称。           |
| `others_color` |        `tuple`        | `(0.7,0.7,0.7,0.5)` |            被合并到 “others” 的类别所使用的 RGBA 颜色。             |
|  `save_path`   | `str \| Path \| None` |       `None`        |            若提供路径，则保存为 HTML 文件；否则直接以交互方式显示。            |


**返回：**

`plotly.graph_objects.Figure`
- 返回生成的 Plotly Figure 对象，便于后续自定义或嵌入。


**算法逻辑：**
1. 若未指定 dim，根据是否存在 Dim3 自动判断为 2D 或 3D；
2. 将标签统一转换为字符串类型；
3. 调用 _categorical_color_mapping 生成类别 → 颜色映射关系；
4. 每个类别生成一个独立的 Plotly trace（2D 使用 Scatter，3D 使用 Scatter3d）；
5. 超出 legend_max 的类别合并为 “others”；
6. 根据 dim 设置二维或三维坐标轴布局；
7. 若提供 save_path，保存为 HTML 文件，否则直接显示。
...
**说明：**
- 本函数不对 embedding 与 labels 进行校验或重排，默认二者已对齐；
- 颜色映射逻辑与 plot_embedding 复用同一套分类颜色规则；
- 每个类别为独立 trace，是交互式图例可控性的基础；
- 适用于 探索性分析、网页展示与 Notebook 交互，不用于出版级静态图。
...
本函数 **仅支持二维 embedding（Dim1 / Dim2）**，不支持三维可视化。
